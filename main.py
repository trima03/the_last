...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email', 'sal'),
        ...
        self.tree.column("sal", width=125, anchor=tk.CENTER)
    ...
    # Добавление данных
    def records(self, name, tel, email, sal):
        self.db.insert_data(name, tel, email, sal)
        self.view_records()

    # Обновление (изменение) данных
    def update_record(self, name, tel, email, sal):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, sal=? WHERE ID=?''',
                          (name, tel, email, sal, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        # добавляем строку ввода для зарплата
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, sal text)''')
        # сохранение изменений БД
        self.conn.commit()

    # метод добавления в БД
    def insert_data(self, name, tel, email, sal):
        self.c.execute('''INSERT INTO db (name, tel, email, sal) VALUES (?, ?, ?, ?)''',
                       (name, tel, email, sal))
        self.conn.commit()
